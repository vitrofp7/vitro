...
public class SensorMLMessageAdapter {
    ...
    static{
        ...
        //TODO: just for the demos! (Not supported by IDAS so it should be removed from here
        idasPhenomenomMap.put(Resource.PHENOMENOM_SWITCH_LIGHT1, idasPhenomenomPrefix + "switchlight1");
        idasPhenomenomMap.put(Resource.PHENOMENOM_SWITCH_LIGHT2, idasPhenomenomPrefix + "switchlight2");
        idasPhenomenomMap.put(Resource.PHENOMENOM_SWITCH_LIGHT3, idasPhenomenomPrefix + "switchlight3");
        idasPhenomenomMap.put(Resource.PHENOMENOM_SWITCH_LIGHT4, idasPhenomenomPrefix + "switchlight4");
    }
    ...
    private String getIdasPhenomenom(Resource resource){
        return idasPhenomenomMap.get(resource.getName());
    }

    private String getIdasUOMCode(String uom){
        return idasUOMMap.get(uom);
    }

    private String getIdasUOMDefinition(String uom){
    ...
    public List<RegisterSensor> getRegisterSensorMessage(CGatewayWithSmartDevices gatewayWithSmartDevicesList) throws VitroGatewayException {
        List<RegisterSensor> registerSensorList = new ArrayList<RegisterSensor>();

        for (CSmartDevice nodeDescriptor : gatewayWithSmartDevicesList.getSmartDevVec()) {
            RegisterSensor tmpRegisterSensor = getRegisterSensorMessage(nodeDescriptor, gatewayWithSmartDevicesList.getGateway());
            if (tmpRegisterSensor!=null)
            {
                registerSensorList.add(tmpRegisterSensor);
            }
        }

        return registerSensorList;
    }

    @SuppressWarnings({ "restriction", "unchecked" })
    public RegisterSensor getRegisterSensorMessage(CSmartDevice nodeDescriptor, CGateway pGatewayInfo) throws VitroGatewayException{
...
//			logger.debug("clonedMessage = " + (RegisterSensor)sensorMLMessageTemplateMap.get(TEMPLATE_REGISTER_SENSOR));
            ...
            List<Identification.IdentifierList.Identifier> identifierList = identification.getIdentifierList().getIdentifier();

            for (Identification.IdentifierList.Identifier identifier : identifierList) {

                Term identifierTerm = identifier.getTerm();

                logger.debug("identifierTerm = {}", identifierTerm.getDefinition());
                ...
                if(identifierTerm.getDefinition().equals(IDAS_UNIVERSAL_IDENTIFIER_LOGICAL_HUB_DEFINITION)){
                    identifierTerm.setValue(pGatewayInfo.getId());
                }
            }

            //******************************************************************************************************************
            // TODO: IMPORTANT replace this with a compatible method call.
            // TODO: Here we have more info than is actually used. But if we inherit a components approach the different sensor models should be useful!
            // TODO: Careful the capabilities in the key set of allmycaps are with their uberdust urns!!!
            List<Integer> sensorModelIdsVec = nodeDescriptor.getSpecificSensorModelIdsVec();
            List<Resource> resourceList = new Vector<Resource>();

            Set<String> allMyCaps = pGatewayInfo.getAllGwGenericCapabilities().keySet();
            boolean tmpfoundmatch;
            for(Integer tmpSensorModelId: sensorModelIdsVec){
                tmpfoundmatch = false;
                for (String tmpCap : allMyCaps)
                {
                    for(int i = 0; i< pGatewayInfo.getAllGwGenericCapabilities().get(tmpCap).size(); i++)
                    {
                        if(pGatewayInfo.getAllGwGenericCapabilities().get(tmpCap).elementAt(i).getSmid() == tmpSensorModelId)
                        {
                            tmpfoundmatch= true;
                            // TODO: Careful, this throws exception if the simple capability name IS NOT in the hashmap dictionary for IDAS.
                            try{
                                resourceList.add(Resource.getResource(tmpCap));
                            }catch (VitroGatewayException ex)
                            {
                                //
                                System.out.println("Unsupported Resource from VITRO");
                            }
                            break;
                        }
                    }
                    if(tmpfoundmatch) break;
                }
            }
            ...
            for (Resource resource : resourceList) {
                //skip registering unsupported resources!
                if(VitroGatewayService.getVitroGatewayService().getUseIdas() &&
                        (resource.getName().equalsIgnoreCase(Resource.PHENOMENOM_SWITCH_LIGHT1) ||
                        resource.getName().equalsIgnoreCase(Resource.PHENOMENOM_SWITCH_LIGHT2)  ||
                        resource.getName().equalsIgnoreCase(Resource.PHENOMENOM_SWITCH_LIGHT3)  ||
                        resource.getName().equalsIgnoreCase(Resource.PHENOMENOM_SWITCH_LIGHT4) )  )
                {
                    continue;
                }
     ...
     //TODO: check what adaptations are needed
    @SuppressWarnings({ "restriction", "unchecked" })
    ...
    private IoComponentPropertyType getOutputComponent(Resource resource){
        ...
        // TODO: special handle for uom for actuators:
        String uomStr =  resource.getUnityOfMeasure();
        if(uomStr==null || uomStr.isEmpty() || uomStr.equals(""))
        {
            uomStr = Resource.UOM_DIMENSIONLESS;
        }
        uom.setHref(getIdasUOMDefinition(uomStr));
        quantity.setUom(uom);
