...
import vitro.vgw.wsiadapter.WSIAdapterCoapHAI;
...
import alter.vitro.vgw.wsiadapter.WsiUberDustCon.VerySimpleSensorMeasurement;

public class WsiHAICoapAdapterCon extends WsiAdapterCon {
private Logger logger = LoggerFactory.getLogger(getClass()); 
	
	private WSIAdapter delegate = new WSIAdapterCoapHAI();
	
	private static WsiHAICoapAdapterCon myCon = null;
     ...
     */
    public static WsiHAICoapAdapterCon getWsiAdapterCon(DbConInfo databaseConnInfo) {
        if (myCon == null) {
            myCon = new WsiHAICoapAdapterCon();
	...
	public CGatewayWithSmartDevices createWSIDescr(CGateway givGatewayInfo) {
        ...
        try{
        	...
        	((WSIAdapterCoapHAI)delegate).connect(null);   // REMOVED BECAUSE OF COMPILATION ISSUES
                ...
                for (ReqSensorAndFunctions currentSensorModel : tmpVecSmAndFuncList) {
                    ...
                    for (ReqFunctionOverData currentRequestedFunction : reqFunctionVec) {
                        ...
                        if (currentRequestedFunction.getfuncId() == ReqFunctionOverData.unknownFuncId) {
                            ...
                            Vector<VerySimpleSensorMeasurement> mySensorReadings = new Vector<VerySimpleSensorMeasurement>();  // bugfix: this is now moved inside the functions loop
                            ...
                            else
                            {
                                ...
                                if(obs != null) {
                                    VerySimpleSensorMeasurement simpleSensorMeasurement = new VerySimpleSensorMeasurement(
