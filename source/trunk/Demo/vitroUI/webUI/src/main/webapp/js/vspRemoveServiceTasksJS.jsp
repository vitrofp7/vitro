...
function RPost(strpost, strmode){
    ...
    else    if (strmode == "ServiceRemove")
        targetHandlerUrl = "<%=request.getContextPath()%>/roleVSP/nRequestVitroServiceRemove.jsp"
    var xmlhttp=null;
...
function removeService(serviceInstance)
	{
	 var targetHandlerUrl = "";
    if(serviceInstance == null )
    {	
    	alert ('The selected service has no ID');
        return;
	}
	 strSend = 'serviceInstance='+ serviceInstance;
    //alert("Refresh the page and cross your fingers");
     //targetHandlerUrl = "<%=request.getContextPath()%>/roleVSP/nRequestGatewayDisable.jsp"
    RPost(strSend, "ServiceRemove");
}
